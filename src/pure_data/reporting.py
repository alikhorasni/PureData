...
class ReportGenerator:
    ...
    def generate_markdown(
        profile: DataProfile, cleaning_steps: Optional[List[str]] = None
    ) -> str:
        ...
        lines.append(
            "| Column | Type | Nulls | Null Ratio | Drift Score | Suggested Rules |"
        )
        lines.append(
            "|--------|------|-------|------------|-------------|-----------------|"
        )
        for col, prof in profile.column_profiles.items():
            ...
            lines.append(
                f"| {col} | {prof.dtype} | {nulls} | {ratio} | {drift} | {rules} |"
            )
