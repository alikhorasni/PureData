...
from .contracts import DataProfile, ColumnProfile, CleanseRule
...
class DynamicProfiler:
    ...
    def generate_profile(self) -> ConcreteDataProfile:
        ...
        # Detect drift for numeric columns by comparing early vs late halves
        ...
        for col in columns:
            ...
            if pa.types.is_numeric(dtype):
                ...
                # Heuristic rule generation based on statistics
                ...
                if (
                    abs(basic_stats.get("skew", 0)) > 1.0
                    or abs(basic_stats.get("kurt", 0)) > 3.0
                ):
    ...
    def _compute_drift_scores(
        ...
        """Compute distribution drift by comparing first half vs second half of the data."""
        if n_rows < 2:
            return {col: None for col in columns}

        half = n_rows // 2
        numeric_cols = [
            col for col in columns if pa.types.is_numeric(schema.field(col).type)
        ]
        if not numeric_cols:
            return {col: None for col in columns}

        # Lazy split: use slice operations to avoid full scan
        early_expr = []
        late_expr = []
        ...
        # Combine all expressions into a single select to reuse the LazyFrame plan
        stats = self._lf.select(early_expr + late_expr).collect().to_dicts()[0]

        drift_scores: Dict[str, Optional[float]] = {}
        for col in numeric_cols:
        ...
        # Non-numeric columns have no drift score
        for col in columns:
